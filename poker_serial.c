...
int main(int argc,char** argv){
	...
	for (int i=0;i<(cnt/numProcesses);i++){
		int cardCount=0;
		while (cardCount<5){
			Card card;
			randomCard(&card);
	...
	MPI_Reduce(&straightFlushes,&totalProcesses,1,MPI_INT,MPI_SUM,0,MPI_COMM_WORLD);
	percent=(float)totalProcesses/(float)cnt*100.0;
	if (rank == 0) {
		printf("We found %d straight flushes out of %d hands or %f percent.\n",straightFlushes,cnt,percent);
		time2 = MPI_Wtime();
		printf("Total time: " + %1.6f, time2-time1);
	}
	MPI_Finalize();
	return 0;
}
